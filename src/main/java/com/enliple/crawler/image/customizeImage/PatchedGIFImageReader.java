...
public class PatchedGIFImageReader extends ImageReader {
	...
	private void outputRow() {
		...
		int width = Math.min(sourceRegion.width, destinationRegion.width * sourceXSubsampling);
		...
		if (updateListeners != null) {
			...
			processImageUpdate(theImage, destX, destY, width, 1, 1, updateYStep, bands);
		}
	}

	private void computeDecodeThisRow() {
		this.decodeThisRow = (destY < destinationRegion.y + destinationRegion.height) && (streamY >= sourceRegion.y) && (streamY < sourceRegion.y + sourceRegion.height) && (((streamY - sourceRegion.y) % sourceYSubsampling) == 0);
	}
	...
	private void startPass(int pass) {
		...
		processPassStarted(theImage, interlacePass, sourceMinProgressivePass, sourceMaxProgressivePass, 0, updateMinY, 1, updateYStep, bands);
	}
