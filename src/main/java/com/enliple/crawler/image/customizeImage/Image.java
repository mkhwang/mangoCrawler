...
/**
 * This is a utility class for performing basic functions on an image, such as retrieving, resizing, cropping, and saving.
 * 
 * @version 1.0
 * @author James H.
 */
public class Image {
	...
	Image(File input) throws IOException, ImageReadException {
		JpegReader jpegReader = new JpegReader(); // 위에서 만든 Class
		img = jpegReader.readImage(input);
		this.sourceType = (sourceType == null ? ImageType.UNKNOWN : sourceType);
	}
	...
	public Image getResizedToHeight(int height) {
		if (height > getHeight())
			throw new IllegalArgumentException("Height " + height + " exceeds height of image, which is " + getHeight());
		int nWidth = height * img.getWidth() / img.getHeight();
		MultiStepRescaleOp rescale = new MultiStepRescaleOp(nWidth, height);
		rescale.setUnsharpenMask(AdvancedResizeOp.UnsharpenMask.Soft);
		BufferedImage resizedImage = rescale.filter(img, null);
	...
	public Image getScale(int width, int height) {

		int type = img.getType() == 0 ? BufferedImage.TYPE_INT_RGB : img.getType(); // BufferedImage.TYPE_INT_ARGB :
																					// img.getType();
		BufferedImage bigImage = new BufferedImage(width, height, type);
		Graphics2D g = (Graphics2D) bigImage.getGraphics();
		g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
		// g.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		g.setComposite(AlphaComposite.Src);
		g.scale((double) width / getWidth(), (double) height / getWidth());
		g.drawImage(img, 0, 0, Color.white, null);
		g.dispose();

		return new Image(bigImage, sourceType);
	}

	public Image getResize(int width, int height) {

		// int type = img.getType() == 0 ? BufferedImage.TYPE_INT_RGB : img.getType(); // BufferedImage.TYPE_INT_ARGB :
		// img.getType();
		BufferedImage bigImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		Graphics2D g = (Graphics2D) bigImage.getGraphics();
		g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
		g.setComposite(AlphaComposite.Src);
		g.drawImage(img.getScaledInstance(width, height, img.SCALE_AREA_AVERAGING), 0, 0, Color.white, null);
	...
	public Image getCropResize720(int width, int height) {
		...
		g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
		g.setComposite(AlphaComposite.Src);
		g.drawImage(img, 0, 0, cropped.getWidth(), cropped.getHeight(), x, y, x + cropped.getWidth(), y + cropped.getHeight(), Color.white, null);
		g.dispose();

		BufferedImage bigImage = new BufferedImage(720, 720, BufferedImage.TYPE_INT_RGB);
		g = (Graphics2D) bigImage.getGraphics();
		g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
		g.setComposite(AlphaComposite.Src);
		g.drawImage(cropped.getScaledInstance(720, 720, img.SCALE_AREA_AVERAGING), 0, 0, Color.white, null);
		g.dispose();

		return new Image(bigImage, sourceType);
	}

	public Image getCropResize(int width, int height) {
		// int type = img.getType() == 0 ? BufferedImage.TYPE_INT_RGB : img.getType(); // BufferedImage.TYPE_INT_ARGB :
		// img.getType();
		int x = getWidth() / 2 - width / 2;
		int y = getHeight() / 2 - height / 2;
		...
		// float scale = 360f / cropped.getWidth();
		// System.out.println("scale : " + scale);
		// width = (int) (cropped.getWidth() * scale);
		// height = (int) (cropped.getHeight() * scale);
		//
		BufferedImage bigImage = new BufferedImage(240, 240, BufferedImage.TYPE_INT_RGB);
		g = (Graphics2D) bigImage.getGraphics();
		g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
		g.setComposite(AlphaComposite.Src);
		g.drawImage(cropped.getScaledInstance(240, 240, img.SCALE_AREA_AVERAGING), 0, 0, Color.white, null);
		g.dispose();
	 ...
	 * 
	 * @param x1
	 *            Starting x-axis position for crop area
	 * @param y1
	 *            Starting y-axis position for crop area
	 * @param x2
	 *            Ending x-axis position for crop area
	 * @param y2
	 *            Ending y-axis position for crop area
	...
	public Image crop(int x1, int y1, int x2, int y2) {
		...
		g.drawImage(img, 0, 0, nNewWidth, nNewHeight, x1, y1, x2, y2, Color.white, null);
		g.dispose();
	...
	/**
	 * Useful function to crop and resize an image to a square. This is handy for thumbnail generation.
	 * 
	 * @param width
	 *            Width of the resulting square
	 * @param cropEdgesPct
	 *            Specifies how much of an edge all around the square to crop, which creates a zoom-in effect on the center of the resulting square. This may be useful, given that when images are
	 *            reduced to thumbnails, the detail of the focus of the image is reduced. Specifying a value such as 0.1 may help preserve this detail. You should experiment with it. The value must be
	 *            between 0 and 0.5 (representing 0% to 50%)
	 ...
	 * 
	 * @param softenFactor
	 *            Strength of softening. 0.08 is a good value
	 * @return New Image object post-softening, unless softenFactor == 0, in which case the same object is returned
	...
	/**
	 * Write image to a file. If the supplied File argument includes an extension, this method will attempt to write the image of that type, IF it is supported. Otherwise JPG is the default. If the
	 * supplied File argument does not include an extension, this method will attempt to write the image in the type of the original source image, IF it is supported. Otherwise JPG is the default.
	 * This method will overwrite a file that exists with the same name
	 * 
	 * @see #getWriterFormatNames()
	 * @param file
	 *            File to write image to
	 * @returns File object representing the image, which will have the proper extension appended if none was supplied (i.e. if the method chose the image type), or will have ".jpg" appended if the
	...
	/**
	 * Write image to a file, specify image type This method will overwrite a file that exists with the same name
	 * 
	 * @see #getWriterFormatNames()
	 * @param file
	 *            File to write image to
